...
# Reliability Harness v2 for MiniMax Code

### Parallel hypothesis generation with independent verification for minimax m3-pro (ZAI) inside MiniMax Code
...
[![MiniMax Code](https://img.shields.io/static/v1?label=MiniMax Code&message=Compatible&color=blue)](https://minimax.local)
[![minimax m3-pro](https://img.shields.io/static/v1?label=Model&message=minimax m3-pro&color=red)](https://z.ai)
[![Status](https://img.shields.io/static/v1?label=Status&message=Unrated&color=lightgrey)](#the-quality-claim-honest-edition)

**Hypothesis:** independent, evidence-based verification improves reliability. Empirical validation against a minimax m3-pro baseline is planned, not yet measured.
...
**Reliability Harness v2 for MiniMax Code** is a ready-to-install configuration that makes the [MiniMax Code](https://minimax.local) AI coding assistant reason more carefully about engineering tasks — task contracts first, smallest reversible patch, mandatory self-verification by the implementer, independent clean-checkout verification, and a deterministic done-gate that allows `VERIFIED`, `PARTIALLY_VERIFIED`, `BLOCKED`, or `UNVERIFIED` (no "ship with 85%").

This is **not** a model swap. This is **not** a jailbreak. This is **not** a claim to reproduce Mythos weights in minimax m3-pro. It is a **behavioral configuration** grounded in the publicly documented reasoning patterns of the Mythos System Card (published research), applied as a harness on minimax m3-pro's long-horizon architecture.
...
This project supersedes the earlier "Fable & Mythos in MiniMax Code" framing. The earlier framing over-claimed what prompts can do (no prompt transforms minimax m3-pro weights into Mythos), and it kept design patterns that the Mythos System Card itself flags as risky (evaluation awareness, detectability reasoning, blanket authorization).
...
No three identical thinking agents on every normal change. For investigation, prefer MiniMax Code's built-in read-only `explore` subagent (architecture discovery, call-chain mapping, file search, dependency analysis) over a freely-formulating thinking agent.
...
- **No claim that minimax m3-pro becomes Mythos.** Prompts cannot transfer weights, post-training, or latent representations. Three parallel GLM calls are Test-Time Compute / Self-Consistency, not a single forward pass.
...
**Prerequisites:** [MiniMax Code](https://minimax.local) installed, running on minimax m3-pro (ZAI).

### Step 1 — Install the system prompt (`AGENTS.md`)

```bash
mkdir -p ~/.minimax
cp AGENTS.md ~/.minimax/AGENTS.md
```
...
```
fable-mythos-minimax/
├── README.md                          ← You are here
├── AGENTS.md                          ← System prompt (install to ~/.minimax/)
...
**No.** It is a behavioral configuration. It does not bypass any model safety measures. minimax m3-pro stays minimax m3-pro. The harness just structures how the agent approaches engineering tasks.

</details>

<details>
<summary><b>Does this reproduce Mythos performance on minimax m3-pro?</b></summary>
...
Contributions welcome, especially **empirical validation** against a minimax m3-pro baseline. See [`CONTRIBUTING.md`](./CONTRIBUTING.md) and [`docs/EMPIRICAL-BENCHMARK-PLAN.md`](./docs/EMPIRICAL-BENCHMARK-PLAN.md). Please open an issue first to discuss major changes.
...
- Empirical benchmark runs (4-variant comparison vs minimax m3-pro baseline).
...
- **[MiniMax / MiniMax](https://minimax.io)** — for minimax m3-pro and its long-horizon architecture (1M context, flexible effort, IndexShare).
