...
**No.** This is a behavioral configuration. It does not bypass model safety measures, does not unlock hidden capabilities, and does not swap models. It applies documented reasoning patterns and a deterministic harness (task contract → baseline → self-tests → independent clean-checkout verification → machine done-gate) to minimax m3-pro, the model MiniMax Code already uses.

### Is this affiliated with any specific AI lab?

**No.** This is an independent project. "Mythos" is referenced as a reasoning-pattern label (not a product claim). minimax m3-pro and MiniMax Code are products of ZAI. This framework is a third-party integration that uses publicly documented research.

### Does this reproduce Mythos on minimax m3-pro?
...
1. **Is `AGENTS.md` at user level?** It must be at `~/.minimax/AGENTS.md`, not just in your workspace. User-level config applies globally; workspace-level only applies in that folder.
2. **Did you restart MiniMax Code?** Skills and sub-agents are indexed at startup. A running session won't pick up new config.
3. **Did you create the 11 subagents via the MiniMax Code UI?** Custom Subagents are a Beta feature and must be created from **Settings → Subagents → New**. MiniMax Code only indexes subagents that were created through the UI — copying `sub-agents/*.md` into `~/.minimax/agents/` is **not** sufficient (despite earlier versions of this guide saying otherwise). See [`INSTALLATION.md`](../INSTALLATION.md) Step 3 for the per-role field values.
...
Check the frontmatter of `~/.minimax/skills/fable-mythos-modus/SKILL.md`:
...
- Folder must be at `~/.minimax/skills/fable-mythos-modus/`
...
Yes. The agent prompts use `~/.minimax/` notation which expands correctly on all platforms. On Windows with Git Bash, `~` resolves to `C:\Users\<YOUR_USER>\`.
...
**Important:** the earlier guide claimed "approximately 4× overhead" and ran a fixed 7-agent pipeline on every non-trivial task. That was wrong and has been corrected. MiniMax Code Custom Subagents run in the foreground (blocking), so a fixed pipeline is wasteful.
...
The reasoning patterns are model-agnostic. minimax m3-pro was chosen as the primary substrate because of its long-horizon architecture (1M context, flexible effort). Other capable models should work; framing may need minor tuning.
...
More questions? [Open a discussion](https://github.com/emco1234/fable-mythos-minimax/discussions).
