...
This document specifies how the Reliability Harness v2 will be empirically validated against a minimax m3-pro baseline. Until this plan is executed, the harness's status is **Unrated — empirical validation pending**. No percentage improvement claims are made.
...
| Variant | Description |
|---|---|
| **V1 — minimax m3-pro baseline** | Plain minimax m3-pro with the platform default system prompt. No harness, no extra instructions. |
...
All four variants use the same minimax m3-pro version, same temperature/effort settings, same task set, same time budget per task.
...
- **Same model version.** Pin minimax m3-pro to a specific released version; record it.
