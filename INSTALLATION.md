...
- **[MiniMax Code](https://minimax.local)** installed and running
- **minimax m3-pro (ZAI)** as the configured model (default for MiniMax Code)
...
3. **11 sub-agents** (5 legacy + 6 new orthogonal reliability agents) — created via the MiniMax Code UI (**Settings → Subagents → New**). Custom Subagents are Beta; you must create each one in the UI, you cannot just copy `.md` files into `~/.minimax/agents/`.
...
## Step 3: Create Sub-Agents via MiniMax Code UI

> **Custom Subagents are Beta and require UI-based creation.** MiniMax Code Custom Subagents are a **Beta** feature. Per the official MiniMax Code docs (https://docs.minimax/local/subagents), you create them from **Settings → Subagents → New**. MiniMax Code then writes the subagent as a Markdown file under `~/.minimax/agents/<name>.md` and loads it on the next run.
>
> **Manually copying `.md` files into `~/.minimax/agents/` is NOT sufficient.** MiniMax Code only indexes a subagent after it has been created through the Settings UI. The `sub-agents/*.md` files in this repo are the **system-prompt source** (the prompt body you paste into the UI), not a drop-in install.

> **Before you start — clear stale files.** If `~/.minimax/agents/` already contains a file named `<name>.md` (e.g. from an earlier install attempt, or files copied from the OpenCode/Grok repos), MiniMax Code will refuse to create that subagent with the error **"agent file '<name>' already exists"**. Run this once to start clean:
>
> ```bash
> # Back up anything currently in ~/.minimax/agents/, then empty it
> mkdir -p ~/.minimax/agents-preinstall-backup
> mv ~/.minimax/agents/*.md ~/.minimax/agents-preinstall-backup/ 2>/dev/null
> ```
>
...
1. Open **Settings → Subagents → New** in the MiniMax Code TUI.
...
3. Click **Save**. MiniMax Code writes `~/.minimax/agents/<name>.md` and indexes the subagent on the next run.
...
Skill für Volltext: ~/.minimax/skills/fable-mythos-modus/SKILL.md
````
...
The table below is a quick index. The authoritative, copy-paste-ready field values are the 11 blocks above. Color choices are suggestions; MiniMax Code may offer a different palette.
...
- Each agent's system-prompt body declares the allowed tools as a **descriptive restriction**, and you must additionally enforce them in the MiniMax Code UI field **Allowed tools** (per the blocks/table above / the Sub-Agent Permission Table in `AGENTS.md`).
...
1. **Fully quit MiniMax Code** (not just close the window — quit the process).
2. **Restart MiniMax Code**.
...
Open a MiniMax Code session and ask:
...
For the investigation phase, prefer the **built-in read-only `explore` subagent** that ships with MiniMax Code (architecture discovery, call-chain mapping, file search, dependency analysis) rather than dispatching a freely-formulating thinking agent. `explore` is purpose-built and cheaper.
...
- Is `SKILL.md` at `~/.minimax/skills/fable-mythos-modus/SKILL.md`?
...
This error means **a file at `~/.minimax/agents/<name>.md` is already reserving that name**, so MiniMax Code refuses to create a second subagent with the same name. Common causes:

- A previous install attempt left documentation-snippet files (no frontmatter) in `~/.minimax/agents/`.
- You copied agent files from another harness (OpenCode/Grok) into `~/.minimax/agents/` — those use a different frontmatter schema and will not load as valid MiniMax Code subagents, but they still reserve the name.
...
mkdir -p ~/.minimax/agents-conflict-backup
mv ~/.minimax/agents/<name>.md ~/.minimax/agents-conflict-backup/

# 3. Restart MiniMax Code, then create the subagent fresh via Settings → Subagents → New
```

To check whether a file is a **valid** MiniMax Code agent or a stale snippet, look at the first line: valid files start with `---` (YAML frontmatter). Files starting with `# Sub-Agent ...` are documentation snippets and should be removed from `~/.minimax/agents/`.

> **Do not** drop the `agents/*.md` files from this repo (or the OpenCode/Grok repos) directly into `~/.minimax/agents/`. They use platform-specific frontmatter (`permission:` object for OpenCode, `permission_mode:` for Grok) that MiniMax Code does not understand, and they will block the matching names. Create the 11 subagents via the UI as described in Step 3.

### Routing doesn't fire as expected

- Check `AGENTS.md` is at `~/.minimax/AGENTS.md` (user-level, not just workspace-level).
...
1. Delete `~/.minimax/AGENTS.md` (or restore your backup).
2. Delete `~/.minimax/skills/fable-mythos-modus/`.
3. Remove the 11 subagents via **Settings → Subagents** (delete each one), or delete the `~/.minimax/agents/<name>.md` files that MiniMax Code wrote when you created them.
4. Restart MiniMax Code.

MiniMax Code returns to its default behavior.
