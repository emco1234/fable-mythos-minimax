# Contributing to Reliability Harness v2 for MiniMax Code
...
The biggest open question: **does independent, evidence-based verification measurably outperform single-pass editing on minimax m3-pro?** We currently have a hypothesis and theoretical grounds — but no measured results yet.

- **Method:** Same prompt, harness enabled vs. disabled, identical minimax m3-pro version, compare success rate and `false_done_rate`.
- **Plan:** See [`docs/EMPIRICAL-BENCHMARK-PLAN.md`](./docs/EMPIRICAL-BENCHMARK-PLAN.md) for the 4-variant comparison (minimax m3-pro baseline / legacy MAP / compact single-agent prompt / Reliability Harness v2) and the task-class matrix.
...
P2/P3 work includes property-based testing, fuzzing, mutation testing, and differential testing agents. See [`docs/RELIABILITY-ROADMAP.md`](./docs/RELIABILITY-ROADMAP.md). If you build one of these as a MiniMax Code sub-agent, please contribute it back.
...
4. **Test** — if you change any of the agent prompts or the skill, verify they still load cleanly in MiniMax Code (frontmatter intact, no encoding issues, agent file at `~/.minimax/agents/<name>.md`).
...
- [ ] Agent prompts load completely in MiniMax Code (no truncation)
- [ ] Routing still fires correctly on a known `complex` task
- [ ] Routing still skips on a known `trivial` task
- [ ] Sub-agents discoverable at `~/.minimax/agents/<name>.md`
...
- **Issues:** [github.com/emco1234/fable-mythos-minimax/issues](https://github.com/emco1234/fable-mythos-minimax/issues)
- **Discussions:** [github.com/emco1234/fable-mythos-minimax/discussions](https://github.com/emco1234/fable-mythos-minimax/discussions)
